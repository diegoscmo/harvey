...
function Descent(x::Array{Float64,1}, valor_res, mult_res::Array{Float64,1},rho::Float64,xl::Array{Float64,1},
                  ...
                  fem_v, fem_f, filt, descent::String, lsearch::String)

  # Search nesta direcao
  if descent == "Steep"
    x, dL, count, n_int = Steepest(x, valor_res, mult_res, rho, xl, xu,
                            max_int, tol_int, count, fem_v, fem_f, filt, lsearch)
  elseif descent == "FR"
...
function LineSearch(x::Array{Float64,1}, mult_res::Array{Float64,1}, rho::Float64, dL::Array{Float64,1},
  ...
  # Search nesta direcao
  if lsearch == "Golden"
    alpha,count = Golden_Search(x, mult_res, rho, dir, xl, xu, tol_int, count, fem_v, fem_f, filt)
elseif lsearch == "Equal"
    alpha,count = Equal_Search(x, mult_res, rho, dir, xl, xu, tol_int, count, fem_v, fem_f, filt)
elseif lsearch == "Back"
    alpha,count = Backtracking(x, mult_res, rho, dL, dir, xl, xu, tol_int, count, fem_v, fem_f, filt)
  else
