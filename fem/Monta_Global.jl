function Global(nelems,ijk,ID,K0,M0,densidades,simp,nforcas,nos_forcas,ngl_efetivos)
    ...
    for j=1:nelems
     ...
     for el = 1:nelems
        ...
        #(0.001 + densidades[el]*(1.0-0.001))^simp
        mfator::Float64 = massadens[el]
                #(0.001 + massadens[el]*(1.0-0.001))
        (glg,gll) = gl_livres_elemento(el,ijk,ID)
         @inbounds  for i = 1:length(glg)
                        ...
                        @inbounds for j = 1: length(glg)
                                  ...
                                  I[contador] = glgi
                                  J[contador] = glgj
       	                          V[contador] = (0.001 + kfator*0.999)*K0[glli,gllj]
                                                # K0[glli,gllj]*kfator
                                  W[contador] = (0.001 + mfator*0.999)*M0[glli,gllj]
                                                # M0[glli,gllj]*mfator
                                  contador = contador + 1
                        end #j
       end #i
...
function Global_KM(nelems,ijk,ID,K0,M0,densidades,simp)
...
function Global_K(nelems,ijk,ID,K0,densidades,simp)
